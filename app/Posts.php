...
class Posts extends Model
{
    ...
    protected $fillable = ['tb1.id','tb1.title','tb1.image','tb1.content','tb1.category_id','tb1.tags','tb1.c_id','tb1.m_id','tb1.d_id','tb1.c_time','tb1.m_time','tb1.d_time'
    ];
    ...
    public function loadListWithPager($params = array())
    {
        ...
        if (isset($params['search_category']) && $params['search_category'] > 0)
            $query->where('tb1.category_id', '=', intval($params['search_category']));
        if (isset($params['search_name']) && strlen($params['search_name']) > 0)
            $query->where('tb1.title', 'like', '%' . $params['search_name'] . '%');
